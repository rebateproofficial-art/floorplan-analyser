...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!apiKey) {
      return NextResponse.json(
        { error: 'API key not configured on server' } as ErrorResponse,
        { status: 500 }
      );
    }
    ...
    try {
      ...
      console.log('Claude API response (first 100 chars):', 
